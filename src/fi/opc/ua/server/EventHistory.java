...
public class EventHistory {
	 ...
	 */
	public void deleteEvents(byte[][] eventIds, StatusCode[] operationResults,
	 ...
	 */
	public Integer readEvents(DateTime startTime, DateTime endTime,
			int maxValues, EventFilter eventFilter,
		...
		ContentFilterDefinition.parseEventFilter(node.getNodeManager()
				.getNodeManagerTable().getNodeManagerRoot(), eventFilter,
				fieldPaths, filterDefinition, eventFilterResult);
		if (startTimeDefined || !endTimeDefined)
			for (int j = 0; j < events.size(); j++) {
				EventData event = events.get(j);
				DateTime t = event.getTime();
				final int compareToEnd = endTimeDefined ? t.compareTo(endTime)
						: -1;
				if (compareToEnd > 0)
					break;
				else {
					...
					if (compareToStart >= 0) {
						if ((i >= firstIndex)
								&& filterDefinition.evaluate(event, true))
							history.add(new HistoryEventFieldList(event
									.getFieldValues(fieldPaths)));
						i++;
						if (history.size() == maxValues)
							// Return continuation point if no more events exist
							// and both timestamps were defined
							return endTimeDefined && (j < events.size()) ? i
									: null;
					}
				}
			}
