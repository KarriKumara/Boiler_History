...
public class MyBigNodeManager extends NodeManager {
	...
	private static final Logger logger = Logger
			.getLogger(MyBigNodeManager.class);
	 ...
	 */
	@Override
	public NodeId getVariableDataType(NodeId nodeId, UaVariable variable)
	 ...
	 * 
	 * @see
	 * com.prosysopc.ua.server.NodeManager#hasNode(org.opcfoundation.ua.builtintypes
	 * .NodeId)
	 */
	@Override
	public boolean hasNode(NodeId nodeId) {
		return nodeId.getValue().equals("MyBigNodeManager")
	...
	protected void afterCreateMonitoredDataItem(ServiceContext serviceContext,
		...
		c.add(item);
		logger.debug("afterCreateMonitoredDataItem: nodeId=" + item.getNodeId()
				+ " c.size()=" + c.size());
	}
	...
	protected void deleteMonitoredItem(ServiceContext serviceContext,
		...
		if (c != null) {
			...
			if (c.isEmpty()) {
				...
				logger.debug("deleteMonitoredItem: monitoredItems size="
						+ monitoredItems.size());
			}
		}
	}
