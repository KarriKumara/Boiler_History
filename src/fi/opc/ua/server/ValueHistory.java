...
class ValueHistory {
	 ...
	 */
	public void deleteAtTimes(DateTime[] reqTimes,
	 ...
	 */
	public void deleteRaw(DateTime startTime, DateTime endTime)
			throws StatusException {
		int i = 0;
		...
		for (int i = 0; i < reqTimes.length; i++) {
			...
			values[i] = new DataValue(v == null ? null : v.getValue(),
					v == null ? new StatusCode(StatusCodes.Bad_NoData) : v
							.getStatusCode(), t, UnsignedShort.ZERO, null,
					null);
		}
		return values;
	 ...
	 */
	public Integer readRaw(DateTime startTime, DateTime endTime, int maxValues,
			boolean returnBounds, int firstIndex, List<DataValue> history) {
		int i = 0;
	...
	private void deleteAtTime(DateTime timestamp) throws StatusException {
		...
		for (int i = values.size() - 1; i >= 0; i--) {
			int compareTo = timestamp.compareTo(values.get(i)
					.getSourceTimestamp());
			if (compareTo == 0) {
				values.remove(i);
				found = true;
	...
	private DataValue getValue(DateTime requestedTime) {
		...
		int i = values.size() - 1;
		while ((i >= 0)
				&& (values.get(i).getSourceTimestamp().compareTo(requestedTime) > 0))
			i--;
